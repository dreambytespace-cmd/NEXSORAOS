...
const FlowArt: React.FC<FlowArtProps> = ({ children, className, id, 'aria-label': ariaLabel = 'Story scroll' }) => {
  ...
  useGSAP(() => {
    ...
    sections.forEach((section, i) => {
      ...
      if (i < sections.length - 1) triggers.push(ScrollTrigger.create({ trigger: section, start: 'bottom bottom', end: 'bottom top', pin: true, pinSpacing: false }));
    });
    ScrollTrigger.refresh();
    return () => triggers.forEach((trigger) => trigger.kill());
  }, { scope: containerRef, dependencies: [childCount(children), reducedMotion] });
  return <main ref={containerRef} id={id} aria-label={ariaLabel} className={cx('w-full overflow-x-hidden', className)}>{children}</main>;
};
